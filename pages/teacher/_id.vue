    ...
    <section class="container">
          ...
          <div class="mt20 ml20">
            ...
            <h3 class="hLh30">
              <span
                class="fsize24 c-333"
              >{{ data.teacher.name }}&nbsp;{{
                data.teacher.level === 1 ? '高级讲师' : '首席讲师'
              }}</span
              >
            </h3>
            <section class="mt10">
              <span class="t-tag-bg">{{ data.teacher.career }}</span>
            </section>
      ...
      <section class="mt30">
        <div>
          ...
          <!-- /无数据提示 开始-->
          <section v-if="data.courses.length === 0" class="no-data-wrap">
...
export default {
  asyncData({ params, error }) {
    return teacher.getOne(params.id).then(({ data: res }) => {
